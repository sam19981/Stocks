...
public class StocksTest {
  ...
  public void BlankTestGetValue() {
    Stock s = StockImpl.getBuilder().stockName("Google").stockSymbol("GOOG").create();
    assertEquals(s.getStockName(), "Google");
    assertEquals(s.getStockSymbol(), "GOOG");
  ...
  public void TestGetValueInvalidStock() {
    Stock s = StockImpl.getBuilder().stockName("Google").stockSymbol("GOO").create();
    s.getValue(LocalDate.now().minusDays(1));
  }

  @Test(expected = IllegalArgumentException.class)
  public void TestFutureValue() {
    Stock s = StockImpl.getBuilder().stockName("Google")
            .stockSymbol("GOOG")
